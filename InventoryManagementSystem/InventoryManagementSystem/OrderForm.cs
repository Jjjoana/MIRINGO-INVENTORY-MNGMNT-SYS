...
namespace InventoryManagementSystem
{
    public partial class OrderForm : Form
    {
        ...
        SqlConnection con = new SqlConnection(@"Data Source=(LocalDB)\MSSQLLocalDB;AttachDbFilename=C:\Users\Joana\Documents\dbMS.mdf;Integrated Security=True;Connect Timeout=30");
        ...
        public void LoadOrder()
        {
            int i = 0;
            dgvOrder.Rows.Clear();
            cm = new SqlCommand("SELECT orderid, odate, O.pid, P.pname, O.cid, C.cname, qty, price, total FROM tbOrder AS O JOIN tbCustomer AS C ON O.cid=C.cid JOIN tbProduct AS P ON O.pid=P.pid WHERE CONCAT (orderid, odate, O.pid, P.pname, O.cid, C.cname, qty, price) LIKE '%"+txtSearchIn.Text+"%'", con);
            con.Open();
            dr = cm.ExecuteReader();
            while (dr.Read())
            {
        ...
        private void dgvUser_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            if (colName == "Delete")
            {
                if (MessageBox.Show("Are you sure you want to delete this order record?", "Delete Record", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                {
                    con.Open();
